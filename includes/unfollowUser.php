...
if (Validate($username)) {
    ...
    if(isset($username_follower) || isset($username)) {
        //Alle data is er
        UnFollow($username_follower, $username);
      } else {
        return false;
      }
} else {
...
function Unfollow($username_follower, $username) {
    ...
    if ($username_follower != $username) {
        ...
        if ($row4 = mysqli_fetch_array($result4)) {
          $query3 = "DELETE FROM users_followers WHERE username = '$username' AND follower = '$username_follower'";
